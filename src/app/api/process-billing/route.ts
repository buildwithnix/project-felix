    ...
    for (const subscription of subscriptions as Subscription[]) {
      ...
      try {
        ...
        if (result.success) {
          ...
          stats.failedCharges++;
          console.log(
            `❌ Failed to charge subscription ${subscription.subscription_id}: ${result.error}`
          );
          if (result.error) {
...
async function processSubscriptionBilling(
  ...
  try {
    ...
    console.log(
      `💳 Creating MIT payment for subscription ${subscription_id} with order ID ${orderId}`
    );
    ...
    console.log(
      `✅ Payment successful for subscription ${subscription_id}, updating next billing date to ${nextBillingDateString}`
    );
...
async function updateSubscriptionStatus(
  ...
  console.log(
    `📝 Updating subscription ${subscriptionId} status to '${status}'${nextBillingDate ? ` with next billing date ${nextBillingDate}` : ''}`
  );
